...
func _ready() -> void:
	print("[BurningHandsPool] Ready (call initialize() from SkillRoot)")
...
func initialize() -> void:
	...
	_initialized = true
	_prepopulate_pool()
	print("[BurningHandsPool] Initialized with %d nodes" % POOL_SIZE)


func _prepopulate_pool() -> void:
	var script_res = preload("res://scripts/skill_system/pools/burning_hands_node.gd")
...
func spawn(caster: Node2D, direction: Vector2, damage: float, damage_type: String, skill_id: String, signal_bus: Node, targets: Array = []) -> Node2D:
	...
	node.initialize(caster, direction, damage, damage_type, skill_id, signal_bus, targets)
	node.visible = true
	_active.append(node)
	return node
